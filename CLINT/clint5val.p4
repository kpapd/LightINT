            ...
            if (hdr.tcp.isValid() && hdr.tcp.srcPort!=9000 && hdr.tcp.dstPort!=9000) { //Control tcp flows not monitored
                ...
                if (array_pos!=0) { //Current flow has an entry in state_array
                    ...
                    if (hdr.swtraces.isValid()) {
                        log_msg("IN swid1 = {},swid2 = {}, swid3 = {}, frwd_state = {}, back_state = {}, dstAddr = {}, dstPort = {}",{hdr.swtraces.swid1,hdr.swtraces.swid2,hdr.swtraces.swid3,frwd_state,back_state, hdr.ipv4.dstAddr, hdr.tcp.dstPort});
                    } else {
                        log_msg("IN NO swid,frwd_state = {}, back_state = {}, dstAddr = {}, dstPort = {}",{frwd_state,back_state, hdr.ipv4.dstAddr, hdr.tcp.dstPort});
                    }   
                    ...
                    */
                    ...
                    if (standard_metadata.ingress_port==ports && frwd_state==SET_UP) { //If in PREPARE phase
                        ...
                        hdr.swtraces.swid3 = BLANK; // Insert BLANK
                        hdr.swtraces.swid4 = BLANK; // Insert BLANK
                        hdr.swtraces.swid5 = BLANK; // Insert BLANK
                    ...
                    if (hdr.swtraces.isValid() && (hdr.swtraces.swid1==RESET || hdr.swtraces.swid2==RESET) && standard_metadata.egress_spec==ports) { // If Reset comes and outputs to host then set opposite direction to Awaiting INIT
                    ...
                    if (back_state == INSERTED && standard_metadata.ingress_port==ports) { // If in "Inserted swID" state and packet comes from host
                        if (!hdr.swtraces.isValid()) { 
                            ...
                            hdr.swtraces.swid3 = hdr.swtraces.swid2; //swid2 is either BLANK (1st if) or thisswid (prev if)
                            hdr.swtraces.swid2 = RESET; 
                        }
                        state_array.write(posRev, AW_INIT); // Set state to Awaiting Init
                        back_state=AW_INIT;
                    } 

                    if (frwd_state == READY_INS && (!hdr.swtraces.isValid() || hdr.swtraces.swid5 == BLANK )) { // My turn to insert swID                    
                        if (!hdr.swtraces.isValid()) {  
                            hdr.swtraces.setValid();
                            hdr.tcp.intflag=1;
                            hdr.ipv4.totalLen = hdr.ipv4.totalLen + PFALENGTH;
                ...
                //if (hdr.swtraces.isValid()){log_msg("Fin OUT swid1 = {}, swid2 = {}, swid3 = {},",{hdr.swtraces.swid1, hdr.swtraces.swid2, hdr.swtraces.swid3});} else {log_msg("Fin OUT no swid");} 
            }
        }
    }
}
