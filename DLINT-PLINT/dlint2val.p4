...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        if (hdr.tcp.isValid()) {
            ...
            if (hdr.swtraces.isValid() && hdr.swtraces.swid1!=INIT && hdr.swtraces.swid1!=RESET && reg_val_fw!=READY_INS) { //Something has gone wrong - Reset
            ...
            if (reg_val_rv == INSERTED && standard_metadata.ingress_port==ports) { // If in "Inserted swID" state and packet comes from host
                if (!hdr.swtraces.isValid()) { 
                    ...
                    bloom_filter.write(reg_pos_fw, READY_INS); //Set state to Ready
                    reg_val_fw=READY_INS;
                }
                bloom_filter.write(reg_pos_rv, AW_INIT); // Set state to Awaiting Init
                reg_val_rv=AW_INIT;
            ...
            if (reg_val_fw == READY_INS && (!hdr.swtraces.isValid() || hdr.swtraces.swid2 == BLANK )) { // My turn to insert swID                    
                if (!hdr.swtraces.isValid()) {  
                    hdr.swtraces.setValid();
                    hdr.tcp.intflag=1;
                    hdr.ipv4.totalLen = hdr.ipv4.totalLen + PFALENGTH;
